...
- ``-i [ --interface ] arg``
    + Description: The ID of the target device/interface. This value MUST be provided to validate the Frontend Options.
        + For QCA9300/IWL5300 NIC, PhyId, DvId, MonId and PhyPath are all acceptable. (What are these IDs? You may refer to :ref:`device_naming` for help.)
...
- ``--freq arg``
    ...
    + Default: The default value is the the current working carrier frequency.
    + Value Range: 
...
- ``--rate arg``
    ...
    + Default: The default value is the the current working baseband bandwidth.
    + Value Range: 
